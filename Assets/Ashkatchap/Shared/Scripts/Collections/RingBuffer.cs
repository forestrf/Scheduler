...
namespace DisruptorUnity3d {
	...
	public class RingBuffer<T> {
		...
		public RingBuffer(ushort capacity) {
			uint capacityPowerOf2 = NextPowerOfTwo(capacity);
			_modMask = capacityPowerOf2 - 1;
			_entries = new T[capacityPowerOf2];
		}
		...
		private static uint NextPowerOfTwo(ushort x) {
			uint result = 2;
			while (result < x) {
				result <<= 1;
			}
